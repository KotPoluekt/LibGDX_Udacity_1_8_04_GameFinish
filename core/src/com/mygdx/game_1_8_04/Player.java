...
public class Player extends InputAdapter {
    ...
    // Moving following the finger mode
    boolean targeting = false;

    // Coordination of the finger to follow him
    Vector2 target;
    ...
    public void update(float delta) {
        ...
        if (targeting) {
            Vector2 difference = new Vector2(target.x - position.x, 0);
            position.x += delta * difference.x * Constants.FOLLOW_MULTIPLIER;
        }

        float accelerometerInput = Gdx.input.getAccelerometerX() / (Constants.GRAVITATIONAL_ACCELERATION * Constants.ACCELEROMETER_SENSITIVITY);
